...
if __name__ =="__main__":
    ...
    plt.title("Original data of the Tokyo Temperature from 2020/06-2021/06")
    plt.xlabel('Time (per hour)', fontsize=8)
    plt.ylabel('Temperature (Celsius degree)', fontsize=12)
    plt.plot(temp_list,label= "Tokyo",color='green')
    plt.legend()
    plt.savefig("../pics/Tokyo_temp.png")
    plt.show()

    # Show the wavelet_transform matrix
    plt.figure(figsize=(16, 5), dpi=80)  # 图片长宽和清晰度
    plt.subplot(121)
    plt.title("WaveLet Transform Matrix of Tokyo")
    ...
    # Show the wavelet_transform matrix
    plt.subplot(122)
    plt.title("WaveLet Power Matrix of Tokyo")
    plt.xlabel('Time (per hour)', fontsize=10)
    plt.ylabel('Freq ( Hz)', fontsize=10)
